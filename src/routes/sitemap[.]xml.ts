...
import { getPublicFormationSlugs } from "@/lib/cms.functions";
  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        try {
          slugs = await getPublicFormationSlugs();
        } catch (error) {
          console.error("sitemap: formations fetch failed", error);
        }
        const entries = [
          ...
          ...slugs.map((p) => ({ path: `/formations/${p.slug}`, priority: "0.7", changefreq: "monthly" })),
        ];
        const urls = entries
          .map(
            (e) =>
